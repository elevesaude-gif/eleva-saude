    ...
    <main className="grid min-h-screen place-items-center bg-[#F7F8FA] px-4 py-10 text-[#0D1B2A]">
      <section className="w-full max-w-xl overflow-hidden rounded-[30px] border border-[#E6E8ED] bg-white shadow-[0_22px_70px_rgba(13,27,42,.1)]">
        ...
        <div className="p-6 sm:p-9">
          {orderNsu && (
            <div className="rounded-2xl border border-[#E6E8ED] bg-[#F7F8FA] p-4">
              <p className="text-[9px] font-bold uppercase tracking-[.14em] text-[#344563]">Número do pedido</p>
              <p className="mt-1 break-all font-mono text-sm font-bold text-[#0D1B2A]">{orderNsu}</p>
            </div>
          )}
          ...
          {receiptUrl && (
            <a href={receiptUrl} target="_blank" rel="noopener noreferrer" className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#047857] px-5 py-3.5 text-sm font-bold text-white shadow-[0_8px_18px_rgba(4,120,87,.2)] transition hover:bg-[#065F46]">
              Abrir comprovante <span aria-hidden>↗</span>
            </a>
          )}

          <div className="mt-5 rounded-2xl border border-[#C9C6F0] bg-[#C9C6F0]/25 p-4 text-xs leading-5 text-[#344563]">
            A confirmação automática por webhook será ativada na próxima etapa. Por enquanto, nossa equipe poderá conferir o pagamento pelos dados desta transação.
          </div>
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      <dt className="font-semibold text-[#344563]">{label}</dt>
      <dd className="mt-1 break-all font-bold text-[#0D1B2A]">{value}</dd>
    </div>
  );
}
