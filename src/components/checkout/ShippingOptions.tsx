...
type Props = { options: ShippingOption[]; selected: string; onSelect: (id: string) => void };

export function ShippingOptions({ options, selected, onSelect }: Props) {
  return (
        ...
        {options.map((option) => {
          ...
          return (
            <label key={option.id} className={`relative flex min-h-40 cursor-pointer flex-col rounded-[20px] border p-4 transition ${active ? "border-[#0D1B2A] bg-[#C9C6F0]/35 shadow-[0_8px_25px_rgba(13,27,42,.08)] ring-2 ring-[#C9C6F0]" : "border-[#E6E8ED] bg-[#F7F8FA] hover:border-[#C9C6F0]"}`}>
              {option.id === "jadlog" && <span className="absolute right-3 top-3 rounded-full border border-[#A7F3D0] bg-[#ECFDF5] px-1.5 py-0.5 text-[7px] font-bold uppercase tracking-wide text-[#047857]">Recomendado</span>}
